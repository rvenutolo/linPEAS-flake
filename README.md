...
- **Security/invariant lints**: `check-jsonschema`, `checkout-persist-credentials`, `dashboard-data-tests`, `dependency-review`, `gitleaks`, `harden-runner-first`, `job-timeout-minutes`, `min-permissions`, `pin-diff-isolated`, `pr-workflows-no-secrets`, `pre-commit-hooks-sha-parity`, `protect-main-drift-check`, `renovate-invariants`, `required-checks-no-paths`, `tag-protection-drift-check`, `upload-artifact-strict`, `uses-sha-pinned`, `workflow-concurrency`.
