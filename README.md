...
| `update-flake-lock.yml`          | weekly Fri + dispatch              | Bumps the flake inputs `nix flake update` can move (the rev-pinned `pre-commit-hooks` input is Renovate's to move); opens auto-merging PR.                        |
...
and tag protection, required-check gating — lives in
