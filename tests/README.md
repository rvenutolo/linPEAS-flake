...
| `LINT_ALLOW_EMPTY_SCAN`              | every script that enumerates through `scripts/lib/enumerate.sh`, which honours the variable without the caller naming it — mostly lints, plus the generators, the doc-freshness runner and the pressure meter — enumerate with `grep -lE 'glob_into\|enumerate_into\|filter_into' scripts/*.sh` | accept a deliberately empty scan set (see `docs/development/linting.md`)                        |
