...
| --------------------- | ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `flake-check`         | `ubuntu-latest`    | `nix flake check` — eval, the pre-commit hook set (treefmt, deadnix, statix, actionlint, yamllint, shellcheck, `lint-shell-tools`, …), and the pinned derivation build (`checks.linpeas-build`); plus `check-flake-systems-eval.sh` (every declared system's packages force-evaluate) and its guard harness |
