...
| --------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `flake-check`         | `ubuntu-latest`    | `nix flake check` — eval, the pre-commit hook set (treefmt, deadnix, statix, actionlint, yamllint, shellcheck, …), and the flake checks (`checks.formatting`, `checks.lint-shell-tools`, and the pinned derivation build `checks.linpeas-build`); plus `check-flake-systems-eval.sh` (every declared system's packages force-evaluate) and its guard harness |
...
`tests/check-<member>.test.sh` fixture harness, where one exists
(`jsonschema` has none, by exemption), before that member's enforcer
and fails on either, so a harness regression reddens the required
context even with a clean tree:
...
| `renovate-invariants`      | `renovate.json` keeps SHA-digest pinning, `minimumReleaseAge`, no top-level `automerge`, and `pinDigests: true` for `github-actions`; every `# renovate:` marker's file is still consumed by a live customManager (`scripts/check-renovate-markers-matched.sh`)                                                                                                                                                                                                                                              |
| `required-checks-no-paths` | No required workflow declares `paths:` / `paths-ignore:` under `pull_request:`                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `lint-workflow-security`   | Batched workflow-security member lints; e.g. member check `uses-sha-pinned`: every `uses:` in workflows + composite actions is a full 40-hex SHA (or a `./...` self-ref). The trailing `# vX.Y[.Z]` patch-tag comment is a separate rule, `patch-tag-pins`, enforced as a pre-commit hook only — it is in no lint group, and `harness-group` runs only its fixture tests, so no required check probes the live tree                                                                                          |
...
| `harness-group`            | Every test harness in the group runs, and `ratchet-pin-audit` additionally runs its enforce script against the live checkout — the only roster entry that does; the rest run no enforce script, though some harnesses still assert live-tree facts — for example the actionlint canaries check that the hook's file filter watches the pin, and the staleness harness's live scenario checks its own check can still read `flake.lock` (pinned off that lock's newest `lastModified`, so no age is asserted) |
| `lint-doc-invariants`      | Batched doc-invariant member lints; e.g. invariant-index integrity, doc anchors, ephemeral references, cron restatement, and schema validation                                                                                                                                                                                                                                                                                                                                                               |
| `lint-script-hygiene`      | Batched shell-hygiene member lints over `scripts/`, `tests/`, and the tooling config those scripts and hooks are declared in; e.g. shebang/pipefail, harness preambles, per-script test coverage, helper routing, and path hygiene                                                                                                                                                                                                                                                                           |
...
| `doc-freshness`                            | Every `refresh-*.sh` generator still reproduces its own output; `refresh-flake-show`, `refresh-pin-parity` and `refresh-ephemeral-refs-gap` also `--check` the committed tree, while the rest regenerate first, so the committed-file gate for those is their `*-fresh` pre-commit hook (see [linting.md](../development/linting.md#generated-document-ownership); the changelog's freshness is `changelog-links`'s). |
| `editorconfig`                             | `.editorconfig` compliance (charset, line endings, trailing whitespace, final newline).                                                                                                                                                                                                                                                                                                                               |
| `lint-pr-title` (workflow `pr-title-lint`) | PR title independently satisfies Conventional Commits and is spell-checked with `typos`. The PR title becomes the merge-commit subject with `" (#<number>)"` appended, and the lint checks that composed string.                                                                                                                                                                                                      |
...
- `image-cve-scan.yml` (weekly cron + push on the paths that change the image + dispatch) runs Trivy and Grype against the released OCI image and uploads SARIF to code-scanning under distinct categories (`trivy-image-cve`, `grype-image-cve`) for cross-scanner DB coverage.
    - Most findings are CVE-DB-driven, not PR-driven, so the scheduled run against a fresh DB is the meaningful signal — and it fires even in weeks with no PR activity. The push trigger covers the other direction: when the image itself changes, waiting for the next Friday leaves the new contents unscanned for up to a week, and no cron ordering helps a bump that lands off-schedule.
    - Both scanners are advisory only (job-level failure is `count > 0` of CRITICAL CVEs or an infrastructure failure ahead of the count; a CRITICAL finding does not stop the SARIF upload); failures auto-file deduped issues split by finding-vs-infrastructure label; the prevention path is a nixpkgs bump via `update-flake-lock`.
- `docs-audit-reminder.yml` (monthly cron + dispatch) opens a deduped `docs-audit` issue when CI-structure churn suggests hand-written CI prose has drifted. Advisory by construction: freshness gates cover only generated content (whole generator-owned files and spliced blocks), and the semantic claims they miss need a reading agent rather than a lint. Non-zero drift pressure never fails the job — a workflow that reddens during normal CI churn trains the maintainer to ignore red — but a genuine failure, such as the pressure script erroring or a GitHub API call failing, does go red, because that is a real problem rather than a drift signal. Drift pressure counts commits touching CI structure since the commit recorded in `.github/docs-audit-state`, so it reads as *change nobody has audited yet*: the issue closes itself once that count is zero, which is what running `just docs-audit-done` in an audit's final fix PR produces. An audit whose point was never recorded leaves pressure climbing and the issue open, which is the correct signal for an audit that never happened. See [Docs correctness audit](../development/docs-audit.md) for the full loop.
- `octoscan.yml`'s PR trigger is paths-filtered to the files it reads (`.github/workflows/**` plus `scripts/octoscan-scan.sh`; composite actions under `.github/actions/` are not scan targets). `codeql.yml` deliberately runs on **every** PR with no paths filter, so the OpenSSF Scorecard SAST check keeps seeing a SAST run on every merged PR (it scores the fraction analysed, and a paths filter would drag that fraction down). Both stay outside the required set: octoscan because `required-checks-no-paths` forbids paths filters on required workflows, codeql so that a single CRITICAL false positive or a CodeQL infrastructure flake cannot wedge every PR.
...
Every job's first step is `step-security/harden-runner` with `egress-policy: block` and a per-job `allowed-endpoints:` allowlist. The eBPF monitor enforces the allowlist and must remain the first step in every job. A missed host appears as a blocked-egress failure and is fixed forward by extending that job's allowlist.
...
flowchart TD
  ...
  data["./scripts/gen-dashboard-data.sh"]
...
| `stale-pin-check`            | `0 9 * * *`     | 09:00 daily  | Auto-file issue when the pin has sat behind upstream past the staleness threshold                                                                                                                                                |
| `ratchet-pin-audit`          | `15 9 * * *`    | 09:15 daily  | Re-derive the canonical SHA of every patch-tagged action pin under `.github/workflows/` (local, self-reference and floating-major pins excluded; composite-action pins are outside the scan); fire on an upstream tag force-move |
...
| `links`                      | `50 5 * * 5`    | Fri 05:50    | Markdown link checker (lychee); cron + dispatch only, not a required check                                                                                                                                                       |
| `codeql`                     | `0 6 * * 5`     | Fri 06:00    | CodeQL static analysis (Actions)                                                                                                                                                                                                 |
| `octoscan`                   | `10 6 * * 5`    | Fri 06:10    | Octoscan SAST on `.github/workflows/`                                                                                                                                                                                            |
| `scorecard-drift-check`      | `20 6 * * 5`    | Fri 06:20    | Run OSSF Scorecard over the curated check set; fail and open a deduped issue when any included check scores below 10                                                                                                             |
| `zizmor-drift-check`         | `30 6 * * 5`    | Fri 06:30    | Re-run zizmor `--min-severity=low` over `.github/workflows/`; open a deduped issue on any finding                                                                                                                                |
...
| `docs-audit-reminder`        | `0 10 1 * *`    | 1st 10:00    | Monthly prompt to run the semantic docs audit; opens a deduped `docs-audit` issue when CI churn since the last recorded audit suggests prose has drifted                                                                         |

Daily crons fire in this UTC order: `ci-watchdog` (00:20) → `actions-cache-prune` (08:00) → `update-linpeas` (08:05) → `stale-pin-check` (09:00) → `ratchet-pin-audit` (09:15) → `flake-lock-staleness-check` (09:45) → `pages` (09:55). `ci-watchdog`'s minute field (`20,50`) recurs every 30 minutes around the clock — the 00:20 timestamp is only where its cron string sorts among the fixed-time daily crons, not a single daily firing. Bump-related crons (`update-linpeas`, `stale-pin-check`) front-load the window so the dashboard cron at 09:55 reads a settled state; drift checks cluster between them. Weekly crons fire Friday in slot order: `update-flake-lock` leads at 05:00 so its auto-merge PR's CI runs inside the window, the build- and release-verification chain (`reproducibility-check` → `image-cve-scan` → `verify-latest-release` → `links`) fills out the first hour, the scanner cluster (`codeql` → `octoscan` → `scorecard-drift-check` → `zizmor-drift-check`) fills the second, and the secret-scan pair (`gitleaks`, `trufflehog`) closes it.

This table is the single source for every schedule: other docs link to it and never restate a literal time or a numeric cadence — `check-doc-cron-restatement.sh` enforces the restatement shape that drifts, a line under `README.md` or `docs/**` that names a workflow and carries a clock time or a numeric cadence (this page and the README's generated `ci-summary` block excepted); linking back to this table is convention, not lint. The table is checked against the workflows' `cron:` lines by `scripts/check-cron-table.sh` — set parity and cron-string accuracy. The daily-order paragraph following the table is diffed against the table's own daily rows, and its times must strictly increase. The script is wired as the `check-cron-table` pre-commit hook. The `cron-table-drift-check` workflow runs the same script on pushes to `main` touching the workflow files, this doc, or the check script (plus manual dispatch).
...
- The dashboard page and the [trust model](../security/trust-model.md) self-describe as documentation, not a trust anchor. Authoritative signal lives in `gh attestation verify` against the published artifacts, not the dashboard text.
...
Triggers: `workflow_run` of `release-on-bump` (any completion) + manual
`workflow_dispatch`. Success filtering happens at the `sync` job:
