...
This table is the single source for every schedule: other docs link to it and never restate a literal time or a numeric cadence — `check-doc-cron-restatement.sh` enforces the restatement shape that drifts, a line under `README.md` or `docs/**` that names a workflow and carries a clock time or a numeric cadence (this page and the README's generated `ci-summary` block excepted); linking back to this table is convention, not lint. The table is checked against the workflows' `cron:` lines by `scripts/check-cron-table.sh` — set parity and cron-string accuracy — and the daily-order paragraph following the table against the table's own daily rows, as an exact match in strictly increasing UTC order; wired as the `check-cron-table` pre-commit hook. The `cron-table-drift-check` workflow runs the same script on pushes to `main` touching the workflow files, this doc, or the check script (plus manual dispatch).
