...
| `renovate-invariants`        | `renovate.json` keeps SHA-digest pinning, `minimumReleaseAge`, per-manager `automerge`, and `pinDigests: true` for `github-actions`; every `# renovate:` marker's file is still consumed by a live customManager (`scripts/check-renovate-markers-matched.sh`)                                                                                                                                                      |
