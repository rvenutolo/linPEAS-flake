...
This table is the single source for every schedule: other docs link to it and never restate a literal time or a numeric cadence — `check-doc-cron-restatement.sh` enforces the restatement shape that drifts, a line under `README.md` or `docs/**` that names a workflow and carries a clock time or a numeric cadence (this page and the README's generated `ci-summary` block excepted); linking back to this table is convention, not lint. The table and the ordering paragraph above it are both checked against the workflows' `cron:` lines by `scripts/check-cron-table.sh` — set parity, cron-string accuracy, and a daily arrow list whose times strictly increase — wired as the `check-cron-table` pre-commit hook. The `cron-table-drift-check` workflow runs the same script on pushes to `main` touching the workflow files, this doc, or the check script (plus manual dispatch).
