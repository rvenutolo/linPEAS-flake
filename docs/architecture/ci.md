...
This table is the single source for every schedule: other docs link to it and never restate a literal time or a numeric cadence — the restatement half is what `check-doc-cron-restatement.sh` enforces; linking back to this table is convention, not lint. The table is itself checked against the workflows' `cron:` lines by `scripts/check-cron-table.sh`, wired as the `check-cron-table` pre-commit hook. The `cron-table-drift-check` workflow runs the same script on pushes to `main` touching the workflow files, this doc, or the check script (plus manual dispatch).
