...
set and the file patterns each handles come from `treefmt.nix`; the
global excludes below are the evaluated set, which adds `treefmt-nix`'s
own defaults to the repo's. It is
