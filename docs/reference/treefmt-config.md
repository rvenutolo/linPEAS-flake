...
This page enumerates the formatters wired into `treefmt`. The formatter
set comes from `treefmt.nix`, as do the file patterns of the formatters
that declare `includes` there — declaring them replaces the module
default rather than adding to it. A formatter that declares none takes
`treefmt-nix`'s patterns. The global excludes below are the evaluated
set, which adds `treefmt-nix`'s own defaults to the repo's. It is
