...
- **The token step fails with `Error: <message>`, preceded by `Failed to create token for "rvenutolo/linPEAS-flake" (attempt N): <message>`** — `actions/create-github-app-token` owns no failure string of its own here; it ends in `core.setFailed(error.message)`, so `<message>` is whatever GitHub returned. Read that, not the wrapper. The step sets neither `owner` nor `repositories`, so it targets this repo and the failing call is `GET /repos/{owner}/{repo}/installation`: a `404` there is the App not being installed on this repo, and a `401` is `SETTINGS_DRIFT_APP_CLIENT_ID` / `SETTINGS_DRIFT_APP_PRIVATE_KEY` mismatched or rotated without updating both halves. The action retries only on `5xx` and network errors, so neither of these is retried. The exact body text GitHub returns for each is not probed here — match on the status, not on a quoted sentence.
