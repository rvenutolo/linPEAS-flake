...
The `prune-stale` job runs every day at 03:00 UTC and on manual
...
Both jobs declare `actions: write` (cache deletion) and `contents: read` (branch enumeration). No other scopes. The fork-guard
