...
- **Changelog tag-pattern parity** — `cliff.toml` `tag_pattern` matches the canonical pin-shape regex enforced across the bump/release/dashboard scripts. → [development/changelog.md](development/changelog.md) <!-- enforcer: scripts/check-cliff-tag-pattern.sh; ci: -; hook: - -->
