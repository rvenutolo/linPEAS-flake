...
- **hammer-shim derivation parity** — `nix/hammer-shim.nix` linpeas derivation must match `flake.nix` linpeas derivation; the shim is a sandbox-required duplicate. → [architecture/auto-update.md](architecture/auto-update.md) <!-- enforcer: scripts/check-hammer-shim-parity.sh; ci: -; hook: hammer-shim-parity -->
...
- **Stale-pin attribution / cron-notify root-cause / dockerhub-sync trigger / Pages invariants / Cron schedule** — cron schedule table in docs/architecture/ci.md must match workflow cron triggers. → [architecture/ci.md](architecture/ci.md) <!-- enforcer: scripts/check-cron-table.sh; ci: -; hook: - -->
...
- **Cache-prune permission scope** — `actions-cache-prune.yml` jobs declare only `actions: write` + `contents: read`. → [development/cache-cleanup.md](development/cache-cleanup.md) <!-- enforcer: scripts/check-min-permissions.sh; ci: min-permissions; hook: min-permissions -->
- **Cache-prune fork-guard** — every job in `actions-cache-prune.yml` includes `github.repository ==` fork-guard. → [development/cache-cleanup.md](development/cache-cleanup.md) <!-- enforcer: scripts/check-fork-guard-release.sh; ci: fork-guard-release; hook: fork-guard-release -->
