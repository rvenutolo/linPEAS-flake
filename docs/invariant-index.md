...
- **Bump-script integrity** — URL prefix, `.digest`, atomic write, API-version header. → [security/verification.md](security/verification.md) <!-- enforcer: scripts/check-gh-api-version-header.sh; ci: lint-script-hygiene; hook: gh-api-version-header -->
...
- **Release-asset blob signatures** — every GitHub Release asset has a sibling `.sigstore` bundle signed via cosign keyless; verified at release time and daily. → [security/verification.md](security/verification.md) <!-- enforcer: -; ci: -; hook: - -->
...
- **ci.yml job ↔ summary category** — every required ci.yml job appears in `ci-check-categories.yml`; every category entry points at a real job. → [security/workflow-hardening.md](security/workflow-hardening.md) <!-- enforcer: scripts/check-ci-job-in-summary.sh; ci: lint-doc-invariants; hook: ci-job-in-summary -->
...
- **Fork-guard on release-grade jobs** — every job with write-scope perms includes `github.repository == 'rvenutolo/linPEAS-flake'` in its `if:`. → [security/workflow-hardening.md](security/workflow-hardening.md) <!-- enforcer: scripts/check-fork-guard-release.sh; ci: lint-workflow-security; hook: fork-guard-release -->
...
- **Ratchet pin audit** — daily detection of tag-vs-SHA drift on pinned action refs; deduped `ratchet-drift` umbrella issue auto-opens/closes. → [runbooks/ratchet-pin-audit.md](runbooks/ratchet-pin-audit.md) <!-- enforcer: scripts/check-ratchet-pin-audit.sh; ci: ratchet-pin-audit-harness; hook: - -->
...
- **Scorecard drift check** — weekly OSSF Scorecard run across a curated 15-check allowlist; any check scoring \<10 fails the workflow and opens a deduped `scorecard-drift` tracking issue. Uses a fine-grained `SCORECARD_PAT` (scoped to linPEAS-flake only, Read-only on Administration/Contents/Metadata/Webhooks) because the Webhooks check exceeds GITHUB_TOKEN scope; rotated annually. → [runbooks/scorecard-pat-rotation.md](runbooks/scorecard-pat-rotation.md) <!-- enforcer: scripts/check-scorecard-threshold.sh; ci: -; hook: - -->
...
- **Cache-prune permission scope** — `actions-cache-prune.yml` jobs declare only `actions: write` + `contents: read`. → [development/cache-cleanup.md](development/cache-cleanup.md) <!-- enforcer: scripts/check-min-permissions.sh; ci: lint-workflow-security; hook: min-permissions -->
