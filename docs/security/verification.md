...
Enforced by `scripts/check-gh-attestation-repo.sh`. The lint joins backslash-continued shell invocations before splitting, then splits every source — runnable lines and backtick spans alike — into one record per occurrence of the command, so a pinned invocation earlier in the same line or span cannot vouch for an unpinned one beside it. The rule applies per record: a record carrying the command plus at least one further token is an invocation and must pass the pin; a record holding the bare command name is a prose mention and is ignored. A runnable line is inspected with its backtick spans removed, so a quoted occurrence is counted once, by the span scan. Fenced code blocks (`sh`, `bash`, `shell`, `console`, `text`, or unlabeled) and inline code spans are both inspected; comment lines in workflows and scripts are skipped before the span scan, and non-shell fences (mermaid, dot) are skipped entirely. Wired as the `lint-workflow-security` CI job (member check `gh-attestation-repo`) and as a pre-commit hook.
