...
Enforced by `scripts/check-gh-attestation-repo.sh`, with parsing in `scripts/_attestation_invocations.awk`. The lint joins backslash-continued shell invocations, then splits every source — runnable lines and backtick spans alike — into shell words, honouring single quotes, double quotes, and backslash escapes, and treating a backtick as a word delimiter rather than part of a word. A record runs from the `gh attestation verify` word triple to the next unquoted shell separator or comment, or to the end of the string. The pin satisfies the check only as a word `--repo` or `-R` whose next word is the slug, or a word `--repo=<slug>`, `-R=<slug>`, or `-R<slug>` — the glued short form `gh` itself accepts.
