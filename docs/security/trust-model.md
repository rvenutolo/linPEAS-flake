...
1. **Forward rules**, mechanical and exact:
    ...
    - A `run:` step invoking the scorecard CLI (`nix develop --command scorecard`, or `scorecard --repo`, matched as either substring) requires `api.securityscorecards.dev`, `api.osv.dev`, and `api.deps.dev` — scorecard queries its own API plus the OSV and deps.dev datasets while evaluating checks. An invocation shaped differently than either keyed substring is not detected.
