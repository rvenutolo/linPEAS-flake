...
6. **SLSA build-provenance + SBOM attestations.** Each release attaches a build-provenance attestation binding the pin file to the `release-on-bump.yml` workflow run; a release that publishes images (`image-mode: full`) additionally binds the per-arch OCI images and attaches CycloneDX-JSON SBOMs (per-arch images) generated by `anchore/sbom-action` and attested via `actions/attest-sbom`. Verifiable with `gh attestation verify` — see [Verification](verification.md).
...
- `docs/security/required-checks.md` restates the `protect-main` branch ruleset's required-check set. Update in same change as GitHub-side list changes.
