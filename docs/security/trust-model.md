...
7. **Sigstore cosign keyless signatures.** Each release additionally signs the bundle (`linpeas-bundle.cosign.bundle` release asset) and every published image digest (per-arch + multi-arch index, both registries) keyless via Sigstore Fulcio + Rekor, identity-bound to `release-on-bump.yml@refs/heads/main`. Exposes the user-facing `cosign verify` / `cosign verify-blob` verification path that does not require the `gh` CLI.
