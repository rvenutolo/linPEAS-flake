...
1. **Top-level `permissions:` is the empty map `{}`.** No scopes are
    granted at workflow scope; every scope must be declared per-job.
...
- For each new job, add a `permissions:` block listing only the scopes
    that job needs. If the job does nothing more than checkout + run
