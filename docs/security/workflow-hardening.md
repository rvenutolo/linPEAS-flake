...
- **The realized shell.** The `lint-script-hygiene` CI job runs the guard from inside `devShells.lint`, validating the shell that actually hosts the batched groups. Only this layer catches a tool that is declared yet does not land on `PATH` — a package whose binary carries a different name, or one shadowed by another shell input. Invoked by hand inside `devShells.default`, the same script confirms the author shell remains a superset; no wired layer runs it there.
