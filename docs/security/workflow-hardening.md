...
Every glob-driven scan fills its array through `glob_into`, the same library's glob analogue. Two positions answer to it: a `for` loop may not expand a pattern at its own loop head, and an array assignment may not expand one in its element list. Either takes an inline `# glob-exempt: <rationale>` marker saying an empty match set is that site's normal state; the rationale is held to the same non-empty rule. Those two are the positions a single pass over the syntax tree decides, and the rule is stated no wider than that — a pattern that reaches a scan by some other route is outside what this lint sees.
