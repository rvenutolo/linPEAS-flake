...
- **The realized shell.** The `lint-script-hygiene` CI job runs the guard from inside `devShells.lint`, validating the shell that actually hosts the batched groups. Only this layer catches a tool that is declared yet does not land on `PATH` — a package whose binary carries a different name, or one shadowed by another shell input. Run inside `devShells.default` — by hand, or through `just verify`, which runs the whole group there — the same script confirms the author shell remains a superset; no CI job or flake check validates that shell.
