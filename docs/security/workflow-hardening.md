...
The rule keys on the `cat --` command itself, which is also the boundary of what it reaches: a payload read spelled `$(<file)`, `mapfile`, a `while read` redirection, or a file operand handed to `jq`/`yq` is invisible to it, and three scripts carry such reads of override-able payloads today (`scripts/check-egress-allowlist.sh`, `scripts/refresh-enforcement-matrix.sh` — which has two, a `while read` redirection and a `yq` file operand — and `scripts/check-permission-scopes.sh`), none carrying a JSON payload the helper could read for them.
