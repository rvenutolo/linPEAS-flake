...
- **The realized shell.** The `lint-script-hygiene` CI job runs the guard from inside `devShells.lint`, validating the shell that actually hosts the batched groups. Only this layer catches a declared tool that the realized shell shadows with another input's binary: the derivation's `PATH` carries nothing but `lintTools`, so a declared package whose binary is absent or renamed already fails there. Run inside `devShells.default` — by hand, or through `just verify`, which runs the whole group there — the same script confirms the author shell remains a superset; no CI job or flake check validates that shell.
