...
What is banned is the same read happening again somewhere a `filter_into` call cannot cover it. A `for` or `while` loop over the narrowed selection that reads the raw `*_FILTER` variable a second time, instead of trusting the selection `filter_into` already handed back, re-derives the same test outside the helper — a filter matching nothing then leaves that read silently false on every iteration, a loop body that never runs, and a clean exit: the exact empty-root failure `filter_into` exists to catch, one step later. A file that reads a `*_FILTER` variable and never calls `filter_into` at all is the same hole with no call site to point to — nothing anywhere in that file asserts the selection the read implies is non-empty. The loop rule covers a read among a `for`'s own iteration words as well as one in its body: either discards just as much of the narrowed set.

A file-scope read outside every loop that only guards a job-count assertion, rather than standing in for `filter_into`, stays legal without a marker: `scripts/check-egress-allowlist.sh` reads `FILE_FILTER` after its own scan loop has finished, to recognize that a fixture harness scanning one workflow at a time is the one legitimate way its notify-job and nix-host-job tallies come back zero. That read guards a count already taken; it does not re-derive a selection a loop body is about to trust, so it carries none of the cardinality risk the banned shapes do.
