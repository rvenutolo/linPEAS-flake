...
# crons, and the required-check context count. Run this ONCE and hand its output
# to every reader, so a path/recipe/output/job/cron named in a doc is checked
# against one authoritative list instead of re-derived per agent. The job list
# and lint-group map are the load-bearing facts: a doc calling a lint-group
# member check a standalone "required CI job" is the drift no freshness gate
# catches.
set -Eeuo pipefail
IFS=$'\n\t'
...
section "CI.YML TOP-LEVEL JOBS (the only real CI jobs / required contexts)"
