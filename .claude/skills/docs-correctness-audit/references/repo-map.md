...
| `docs/_data/dashboard.yml` (untracked/gitignored — regenerated by `pages.yml` in the step before the site build and by `just site-data` / `just site-dev`; the `#site` derivation consumes it rather than producing it; drives `dashboard.md`, `releases.md`; no freshness gate — `dashboard-data-tests` covers generator failure modes only) | `just site-data` / `gen-dashboard-data.sh`                                                                                   |
